...
1. `init-project.sh` — content is in the Init Script section below, make it executable
...
directory (where init-project.sh lives). No heredoc duplication needed.
...
- `init-project.sh` — is executable, copies `fetch-skill.sh` and `skill-catalog.md`
- Running `bash init-project.sh` produces a project with:
